...
def main(args):
    ...
    if args.algo in ["kd-semi"]:
        ...
        checkpoint = torch.load(f'/home/XXX/data/teacher/{args.dataset}/wrn-{args.depth}-{args.widen_factor}_teacher-{args.teacher}_numlabels-{4000}.pth.tar'
    ...
    if args.algo in ["kd-sup"]:
        ...
        checkpoint = torch.load(f'/home/XXX/data/teacher/{args.dataset}/wrn-{args.depth}-{args.widen_factor}_teacher-{args.teacher}_numlabels-{50000}.pth.tar'
                                , map_location='cuda:' + str(args.gpu))
        #teacher_model.load_state_dict(checkpoint['ema_state_dict'])
        teacher_model.load_state_dict(checkpoint['state_dict'])
        teacher_model.eval()

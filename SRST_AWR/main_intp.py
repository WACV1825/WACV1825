...
from loss_intp import *
from attacks.pgd_intp import PGD_Linf, GA_PGD
from attacks.earlystop import Early_PGD

from util import myLogger, backup_codes
import wandb
...
parser.add_argument('--resume-dir', default='', type=str, metavar='PATH', help='path to latest checkpoint (default: none)')

########################## basic setting ##########################
parser.add_argument('--seed', type=int, default=0, help='seed')
parser.add_argument('--gpu', default=0, type=int, help='id(s) for CUDA_VISIBLE_DEVICES')
parser.add_argument('--data_dir', default='data/datasets', help='Directory of dataset')
...
parser.add_argument('--save_dir', type=str, default='experiments/semi-arow', help='save directory')
...
parser.add_argument('--teacher-dir', type=str, default='', help='directory of teacher model')
...
parser.add_argument('--wandb-run-name', type=str, help='save directory')
parser.add_argument('--wandb-entity', type=str, help='wandb entity')
parser.add_argument('--wandb-project', type=str, help='wandb project')

############################# added #############################
parser.add_argument('--tau-margin', type=float, default=2.0, help='temperature of margin computation')
parser.add_argument('--lr-setup', type=int, default=2, help='0: lr = [0.1, 0.01, 0.001, 0.0005] decay at [60, 70, 90]; \
                                                             1: lr = [0.1, 0.01, 0.001, 0.0001] decay at [75, 90, 100]; \
                                                             2: lr = [0.1, 0.01] decay at [50, 150]')
...
parser.add_argument('--intp-steps', type=int, default=3, help='steps for binary search')
parser.add_argument('--mixed', action='store_true', default=False, help='use both intp and adv')
parser.add_argument('--mixed-beta', type=float, default=None, help='mix factor')
parser.add_argument('--inner-target', type=str, default='hard', help='target used in inner maximization: `hard` or `soft`')
...
def main(args):
    ...
    out_directory += f'_{args.wandb_run_name}'

    if not os.path.isdir(out_directory):
        mkdir_p(out_directory)

    with open('experiments/run_names.txt', 'a') as f:
        f.write('Run {} is located in {}\n'.format(args.wandb_run_name, out_directory))

    backup_codes(out_directory)

    log = myLogger(path=out_directory)
    log.info(str(args))

    if args.wandb:
        wandb.init(project=args.wandb_project, entity=args.wandb_entity, name=args.wandb_run_name)
        log.info('wandb name: {}'.format(args.wandb_run_name))
    ...
    teacher_dir = teacher_dirs[args.dataset]
    ...
    if args.train_attack == 'pgd_linf':
        ...
        raise ValueError('not supported train_attack')
    ...
    if args.lr_scheduler == "MultiStep":
        if args.swa:
            if args.lr_setup == 0:
                scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[60, 70, 90], gamma=0.1)
            elif args.lr_setup == 1:
                scheduler = lr_scheduler.MultiStepLR(optimizer, milestone=[75, 90, 100], gamma=0.1)
            if args.lr_setup == 2:
    ...
    if args.resume:
        ...
        if args.swa:
            try:
                ...
                swa_model, _ = create_model(args.swa)
                swa_n = 0
        del checkpoint
        torch.cuda.empty_cache()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.swa and epoch >= args.swa_start and (epoch - args.swa_start) % args.swa_c_epochs == 0:
            ...
            if epoch % 2 == 0:
                ...
                log.info('[Epoch {}] Test: Average loss: {:.4f}, Accuracy: {:.0f}%, Robust Accuracy: {:.0f}%'.format(
                    round(epoch), test_loss, test_acc, attack_test_acc))
                if args.wandb:
                    wandb_dict['epoch'] = round(epoch)
                    wandb_dict['train_loss'] = train_loss
                    wandb_dict['eval_test_loss'] = test_loss
                    wandb_dict['eval_test_accuracy'] = test_acc / 100
                    wandb_dict['eval_test_robust_loss'] = attack_test_loss
                    wandb_dict['eval_test_robust_accuracy'] = attack_test_acc / 100
            ...
            if epoch == 1 or epoch % 5 == 0 or epoch >= args.epochs - 20 :
                ...
                logger.append([round(epoch), train_loss, test_loss, test_acc, attack_test_loss, attack_test_acc])
                # 'train_loss': loss at training time
                # 'test_loss', 'test_acc': eval natural
                # 'attack_test_loss', 'attack_test_acc': eval adversarial
                if args.wandb:
                    wandb_dict['epoch'] = round(epoch)
                    wandb_dict['train_loss'] = train_loss
                    wandb_dict['eval_test_loss'] = test_loss
        ...
        if args.wandb:
            wandb_dict['best_accuracy'] = best_acc / 100
            wandb_dict['best_robust_accuracy'] = attack_best_acc / 100
...
def train(args, labeled_trainloader, unlabeled_trainloader, lb_dataset, ulb_dataset, epoch, model, optimizer, use_cuda, attack, teacher_model=None):
    adjust_rho(epoch)
    ...
    print(len(lb_dataset), len(ulb_dataset))

    for batch_idx, (idx_ul, inputs_ul_w, inputs_ul_s) in enumerate(unlabeled_trainloader):

        data_time.update(time.time() - end)
        batch_size = inputs_ul_w.size(0)
        
        try:
            idx_lb, inputs_lb, targets_lb = next(labeled_train_iter)
        except:
            labeled_train_iter = iter(labeled_trainloader)
            idx_lb, inputs_lb, targets_lb = next(labeled_train_iter)
        ...
        if use_cuda:
            ...
            inputs_lb, targets_lb = inputs_lb.cuda(), targets_lb.cuda(non_blocking=True)
     
        with amp_cm():
           
            if args.algo == "srst-awr":
                sup_loss, reg_loss, know_loss, alpha_r, perturb_linf = SRST_AWR_KD_loss(inputs_lb,
                                                                        targets_lb,
                                                                        inputs_ul_w,
                                                                        model,
                                                                        attack,
        ...
        # plot progress
        info = '({batch:>3}/{size:>3}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | Sup_loss: {sup_loss:.4f} | Reg_loss: {reg_loss:.4f} | Know_loss: {know_loss:.4f}  | Tot loss:{loss:.4f}'.format(
                    batch   = batch_idx + 1,
...
def validate(val_loader, model, criterion, use_cuda, mode, attack=None):
    ...
    for batch_idx, (inputs, targets) in enumerate(val_loader):
        ...
        if attack is not None:
            adv_inputs, _ = attack.perturb(inputs, targets=targets)
        ...
        # plot progress
        info = '({batch:>3}/{size:>3}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f}'.format(
                        batch=batch_idx + 1,
                        size=len(val_loader),
                        data=data_time.avg,
                        bt=batch_time.avg,
...
def adjust_max_eps(epoch):
    ...
    if ges['mode'] == 'const':
        ...
        if epoch < ges['t']:
            max_eps = args.eps / 255 * (epoch / ges['t'])
        else:
            max_eps = args.eps / 255
    elif ges['mode'] == 'curious':
        if epoch < ges['t']:
            max_eps = args.eps / 255 * (ges['gamma'] * epoch / ges['t'])
        else:
            max_eps = args.eps / 255
    elif ges['mode'] == 'curious_smooth':
        if epoch < ges['t']:
            ...
            max_eps = args.eps / 255 * (ges['gamma'] - 0.1 * (ges['gamma'] - 1) * (epoch - ges['t']))
        else:
            max_eps = args.eps / 255 
    else:

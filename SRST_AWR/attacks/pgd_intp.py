...
class PGD_Linf():
    ...
    def perturb(self, x_nat, epsilon=None, targets=None):
        ...
        if self.epsilon is None:
            if self.random_start:
                # epsilon should be a scalar value
                # x_adv = x_nat.detach() + epsilon * torch.randn(x_nat.shape).cuda().detach()
                x_adv = x_nat.detach() + torch.empty_like(x_nat).uniform_(-epsilon, epsilon).cuda().detach()
                x_adv = torch.clamp(x_adv, min=0.0, max=1.0).detach()
            else:
        ...
        for _ in range(self.num_steps):
            ...
            if self.epsilon is None:
                ...
                d_adv = torch.clamp(x_adv - x_nat, min=-epsilon, max=epsilon).detach()

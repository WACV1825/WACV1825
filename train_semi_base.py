...
parser = argparse.ArgumentParser(description='PyTorch Semi-Supervised Adversarial Training (Baseline)')
...
if teacher_dir:
    ...
    labeled_per_class = args.num_labels_per_class  
    for i in range(num_classes):
        indices = np.where(np.array(full_dataset.targets) == i)[0]
        np.random.shuffle(indices)
        labeled_indices.extend(indices[:labeled_per_class])
        unlabeled_indices.extend(indices[labeled_per_class:])
...
def train(args, model, model_c, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target, indices) in enumerate(train_loader):
        ...
        if args.outer in ['rst']:
            ...
            loss_robust = (1.0 / bs) * nn.KLDivLoss(size_average=False)(F.log_softmax(model(x_adv), dim=1),
                                                        torch.clamp(F.softmax(logits, dim=1), min=1e-8))
            loss = loss_sup + args.lambd * loss_robust

        elif args.outer in ['uat']:
            logits_adv = model(x_adv)
            loss_sup = F.cross_entropy(logits_adv, target)
            loss_robust = (1.0 / bs) * nn.KLDivLoss(size_average=False)(F.log_softmax(logits_adv, dim=1),
                                                        torch.clamp(F.softmax(model(data).detach(), dim=1), min=1e-8))
            loss = loss_sup + args.lambd * loss_robust

        else:
            raise ValueError('args.outer must be rst or uat')
